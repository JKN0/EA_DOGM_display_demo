...
void LiquidCrystal_74HC595::begin(uint8_t cols, uint8_t rows, uint8_t charsize) {
    ...
    // Reset display
    bitWrite(_register, 6, 0);
    transfer();
    delay(50);
    bitWrite(_register, 6, 1);
    transfer();
    ...
    if (_rows > 1) {
        command(LCD_FUNCTIONSET | LCD_4BITMODE | LCD_2LINE | LCD_5x8DOTS | LCD_IS1);  // 0x29: select instruction table 1
    ...
    // EADOGM162 display extended commands in IS 1
    command(LCD_BIAS);
    command(LCD_POWER);
    command(LCD_FOLLOWER);
    command(LCD_CONTRAST);
    // Return to instruction table 0
    command(LCD_FUNCTIONSET | LCD_4BITMODE | LCD_2LINE | LCD_5x8DOTS | LCD_IS0);  // 0x28
    
    command(LCD_DISPLAYCONTROL | _displayControl);  // 0x0C
    clear();                                        // 0x01
    command(LCD_ENTRYMODESET | _displayMode);       // 0x06
}
...
void LiquidCrystal_74HC595::backlight(uint8_t state) {
    bitWrite(_register, 7, state);
    transfer();
}
